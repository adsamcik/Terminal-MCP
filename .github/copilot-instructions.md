...
- Idle cleanup is opt-in via the `TERMINAL_MCP_IDLE_TIMEOUT` env var (integer seconds or humantime like `1h`); unset or unparsable values leave cleanup disabled, and `server::run` never fails startup on a bad value.
